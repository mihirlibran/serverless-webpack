...
module.exports = {
  validate() {
    ...
    if (this.options.function) {
      ...
      if (serverlessFunction.runtime && serverlessFunction.runtime.startsWith('node')) {
      ...
      _.forEach(functions, (func, index) => {
        const functionDefinition = this.serverless.service.getFunction(func);
        if(!functionDefinition.runtime.startsWith("java")){
           const entry = getEntryForFunction.call(this, functions[index], functionDefinition);
          _.merge(entries, entry);
        }
      });
    }
